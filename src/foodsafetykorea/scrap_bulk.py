...
class Crawler:
    ...
    def crawl_prod_info(self):
        ...
        ingredient_info_df.to_parquet(f"{COLLECT_PATH}/ingredient/{self.query_roman}_{prod_info_agg['품목보고번호']}.parquet")
    ...
    def crawl_data_from_page(self):
        ...
        for item_page in df.item_page:

            page_success = False
            for _ in range(20):
                try:
                    file_path = f"{COLLECT_PATH}/product/{self.query}_{item_page}.json"
            ...
            if page_success is False:
                raise Exception(f"{self.cur_page} 페이지 상품 크롤 10번 실패")

            ## 이번 iteration 끝
            self._print(f"{prod_info_agg['품목보고번호']} 상품 회사 정보 크롤 완료")
    ...
    def to_parquet_df_list(self, query):
        ...
        df.to_parquet(f"./crawled/{self.query_roman.replace(' ', '_')}.parquet")
        self._print("저장 완료")
